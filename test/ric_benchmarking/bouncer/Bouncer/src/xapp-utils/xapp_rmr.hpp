...
template <class MsgHandler> void XappRmr::xapp_rmr_receive(MsgHandler&& msgproc, XappRmr *parent){
	...
	while(parent->get_listen()) {    
		mdclog_write(MDCLOG_INFO, "Listening at Thread: %s <file:%s>",  thread_id.str().c_str(),__FILE__);
		...
		num++;
        mdclog_write(MDCLOG_DEBUG, "Total Responses received : %d", num);
		...
		else
		{
			...
			if(*resend){
				mdclog_write(MDCLOG_INFO,"RESENDING to Sender Message of Type: %d",this->_xapp_received_buff->mtype);
				mdclog_write(MDCLOG_INFO,"RESENDING to Sender Message: %s",(char*)this->_xapp_received_buff->payload);
...
/* Added for CONTROL_ACKNOWLEDGEMENT - STARTED*/				
				mdclog_write(MDCLOG_INFO,"CONTROL_REQ has been send successfully, <file= %s, line=%d >",__FILE__,__LINE__);
